...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
        ...
        <div class="min-h-screen">
            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="mt-8 bg-black-100 dark:bg-gray-800 overflow-hidden shadow sm:rounded-lg">
                    <div id="content_categories"></div>
                    <section class="container galeria">
                        <div id="content_images" class="row"></div>
                    </section>
                </div>
    ...
    <script>
        ...
        async function getCategories() {
            ...
            if(categories.status == 200){
                ...
                data_categories.forEach(element => {
                    ...
                    console.log("llego cat = ", element)
                    html_categories += '<button type="button" class="btn btn-categories" onclick="filter('+element.value+')">'+element.label    +'</button>'
                });
            }
        ...
        async function getImages() {
            console.log("llego")
            const response = await fetch("/api/v1/imageproducts/ES/0/0");
            const images = await response.json();
            let html_imgs = ''
            if(images.status == 200){
                data_images = images.data
                console.log('imagenes = ', data_images)
